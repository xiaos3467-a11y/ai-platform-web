...
const Prompts: React.FC = () => {
  ...
  const fetchVersions = async (promptId: string) => {
    try { const resp = await api.get<PromptVersion[]>(`/prompts/${promptId}/versions`); setVersions(resp.data || []); } catch { /* */ }
  };
  ...
  const handleRender = async () => {
    ...
    try {
      ...
      const resp = await api.post<{ rendered: string }>(`/prompts/${selectedPrompt.id}/render`, { variables: vars });
  ...
  const columns = [
    ...
    { title: '', width: 160, render: (_: unknown, record: PromptTemplate) => (
      <Space>
        ...
        <Button size="small" icon={<HistoryOutlined />} onClick={() => { setSelectedPrompt(record); setVersionsOpen(true); fetchVersions(record.id); }} style={{ borderRadius: 8, fontSize: 12 }}>
          版本
        </Button>
      </Space>
    )},
  ...
  return (
    <div>
      ...
      {/* Create Modal */}
      <Modal title="创建 Prompt 模板" open={createOpen} onCancel={() => setCreateOpen(false)} onOk={() => form.submit()} width={700} okText="创建" cancelText="取消">
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          <Form.Item name="name" label="名称" rules={[{ required: true }]}><Input placeholder="如：客服回复模板" /></Form.Item>
          <Form.Item name="description" label="描述"><Input placeholder="模板用途描述..." /></Form.Item>
          <Form.Item name="content" label="模板内容 (Jinja2)" rules={[{ required: true }]} extra="支持 {{variable}}、{% if %}、{% for %} 语法">
            <TextArea rows={10} placeholder="你是一个{{role}}。请回答：{{question}}" style={{ fontFamily: 'monospace', borderRadius: 10 }} />
          </Form.Item>
        </Form>
      </Modal>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => { newVersionForm.resetFields(); setNewVersionOpen(true); }} style={{ borderRadius: 10 }}>
            创建新版本
          </Button>
        </div>
        {versions.length === 0 ? (
      ...
      >
        <Form form={newVersionForm} layout="vertical" onFinish={handleCreateVersion}>
          <Form.Item name="content" label="模板内容" rules={[{ required: true, message: '请输入模板内容' }]}>
            <TextArea rows={10} placeholder="模板内容..." style={{ fontFamily: 'monospace', borderRadius: 10 }} />
          </Form.Item>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Button type="primary" onClick={handleRender} icon={<SwapOutlined />} loading={renderLoading} style={{ borderRadius: 10, fontWeight: 500 }}>
            渲染
          </Button>
          <div>
