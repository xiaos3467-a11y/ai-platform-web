...
import {
  ...
  KeyOutlined, LockOutlined, TeamOutlined,
...
interface UserItem {
  ...
  phone: string | null; is_active: boolean; is_superadmin: boolean;
  roles: { id: string; name: string }[]; last_login_at: string | null; created_at: string;
}
interface RoleItem { id: string; name: string; description: string | null; }
...
const Users: React.FC = () => {
  ...
  const handleCreate = async (values: { username: string; email: string; password: string; display_name?: string; phone?: string; role_ids?: string[] }) => {
    try { await api.post('/users/', values); message.success('用户创建成功'); setCreateOpen(false); form.resetFields(); fetchData(); } catch { /* */ }
  };

  const handleEdit = async (values: { display_name?: string; email?: string; phone?: string; is_active?: boolean; role_ids?: string[] }) => {
    if (!editUser) return;
    try { await api.put(`/users/${editUser.id}`, values); message.success('用户更新成功'); setEditUser(null); editForm.resetFields(); fetchData(); } catch { /* */ }
  };
  ...
  const handleDelete = (id: string) => {
    modal.confirm({
      ...
      cancelText: '取消',
      onOk: async () => { await api.delete(`/users/${id}`); message.success('用户已删除'); fetchData(); },
    });
  };

  const handleToggleActive = async (user: UserItem) => {
    try { await api.put(`/users/${user.id}`, { is_active: !user.is_active }); message.success(user.is_active ? '已禁用' : '已启用'); fetchData(); } catch { /* */ }
  };

  const handleResetPassword = async (values: { new_password: string; confirm_password: string }) => {
    if (!resetPwdUser) return;
    if (values.new_password !== values.confirm_password) { message.error('两次输入的密码不一致'); return; }
    try { await api.post(`/users/${resetPwdUser.id}/reset-password`, { new_password: values.new_password }); message.success('密码已重置'); setResetPwdUser(null); resetPwdForm.resetFields(); } catch { /* */ }
  };
  ...
  const columns = [
    { title: '用户', dataIndex: 'username', render: (name: string, record: UserItem) => (
      <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{
          ...
          background: record.is_superadmin ? 'linear-gradient(135deg, #ff453a, #ff6961)' : 'linear-gradient(135deg, #0a84ff, #5e5ce6)',
    ...
    { title: '操作', width: 120, render: (_: unknown, record: UserItem) => (
      <Space>
        ...
        ><EditOutlined /></div>
        <div onClick={() => { setResetPwdUser(record); resetPwdForm.resetFields(); }}
          style={{ cursor: 'pointer', fontSize: 14, color: 'rgba(255,255,255,0.3)', padding: 4, borderRadius: 6, transition: 'all 0.2s' }}
          onMouseEnter={(e) => { e.currentTarget.style.color = '#ffd60a'; e.currentTarget.style.background = 'rgba(255,214,10,0.1)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.color = 'rgba(255,255,255,0.3)'; e.currentTarget.style.background = 'transparent'; }}
        ><KeyOutlined /></div>
    ...
    <div>
        ...
        >
          {users.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 24px' }}>
              ...
              <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.3)', fontWeight: 500, marginBottom: 8 }}>还没有用户</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.15)', marginBottom: 24 }}>添加用户来使用系统功能</div>
              <Button type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>添加第一个用户</Button>
            </div>
          ) : (
      ...
      <Modal title="添加用户" open={createOpen} onCancel={() => { setCreateOpen(false); form.resetFields(); }} onOk={() => form.submit()} width={500} okText="添加" cancelText="取消">
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="phone" label="手机号"><Input placeholder="13800138000" /></Form.Item>
      ...
      {/* Edit Modal */}
      <Modal title="编辑用户" open={!!editUser} onCancel={() => { setEditUser(null); editForm.resetFields(); }} onOk={() => editForm.submit()} okText="保存" cancelText="取消">
        <Form form={editForm} layout="vertical" onFinish={handleEdit}>
          <Form.Item name="display_name" label="显示名称"><Input /></Form.Item>
          <Form.Item name="email" label="邮箱"><Input /></Form.Item>
          <Form.Item name="phone" label="手机号"><Input /></Form.Item>
          <Form.Item name="is_active" label="启用状态" valuePropName="checked"><Switch checkedChildren="启用" unCheckedChildren="禁用" /></Form.Item>
          <Form.Item name="role_ids" label="角色">
        ...
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {detailUser && (
              <div style={{
                width: 30, height: 30, borderRadius: 10,
                background: detailUser.is_superadmin ? 'linear-gradient(135deg, #ff453a, #ff6961)' : 'linear-gradient(135deg, #0a84ff, #5e5ce6)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: '#fff', fontSize: 13, fontWeight: 600,
              }}>
