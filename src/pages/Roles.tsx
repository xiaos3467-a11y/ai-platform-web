...
import { Table, Button, Modal, Form, Input, Space, Typography, App, Checkbox, Divider } from 'antd';
...
const Roles: React.FC = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (v: number) => <span style={{ color: 'var(--text-muted)' }}>{v} 人</span>,
      },
      {
        title: '创建时间',
  ...
  const renderPermissionMatrix = () => (
    <div style={{ overflowX: 'auto', borderRadius: radius.md, border: `0.5px solid ${matrixBorder}` }}>
          ...
          <tr style={{ background: 'var(--bg-card)' }}>
            <th
              style={{
                padding: '10px 14px',
                textAlign: 'left',
                borderBottom: `0.5px solid ${matrixBorder}`,
                minWidth: 120,
              }}
            >
              <Checkbox
                ...
                indeterminate={
                  selectedPermIds.length > 0 && selectedPermIds.length < permissions.length
                }
                onChange={(e) => toggleAll(e.target.checked)}
              />
              <span style={{ marginLeft: 8, color: 'var(--text-soft)', fontWeight: 500 }}>
                资源 / 操作
              </span>
            </th>
            {actions.map((action) => (
              ...
              >
                <Checkbox
                  checked={permissions
                    ...
                    .every((p) => selectedPermIds.includes(p.id))}
                  indeterminate={
                    permissions
                      .filter((p) => p.action === action)
                      .some((p) => selectedPermIds.includes(p.id)) &&
                    !permissions
                      .filter((p) => p.action === action)
                      .every((p) => selectedPermIds.includes(p.id))
                  }
                ...
                />
                <div
                  style={{ marginTop: 4, fontWeight: 500, color: 'var(--text-dim)', fontSize: 12 }}
                >
        ...
        <tbody>
          {resources.map((resource, idx) => (
            <tr
              key={resource}
              style={{
                borderBottom: idx < resources.length - 1 ? `0.5px solid ${matrixBorder}` : 'none',
              }}
            >
              <td style={{ padding: '10px 14px' }}>
                ...
                />
                <span style={{ marginLeft: 8, fontWeight: 500, color: 'var(--text-primary)' }}>
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          <SectionCard title="权限矩阵总览">
            <div
              style={{ overflowX: 'auto', borderRadius: radius.md, border: `0.5px solid ${matrixBorder}` }}
            >
                ...
                <tbody>
                  {roles.map((role, idx) => (
                    <tr
                      ...
                      style={{
                        borderBottom:
                          idx < roles.length - 1 ? `0.5px solid ${matrixBorder}` : 'none',
                      }}
                    >
                      <td
                        style={{
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Divider style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-subtle)' }}>
            权限配置
          </Divider>
      ...
      >
        <Form form={editForm} layout="vertical" onFinish={handleEdit}>
          <Form.Item name="name" label="角色名称">
            <Input disabled={editRole?.is_system} />
          </Form.Item>
          <Form.Item name="description" label="描述">
