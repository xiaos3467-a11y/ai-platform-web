...
const AdminTenants: React.FC = () => {
  ...
  const { data: tenantDetail } = useApiQuery<Tenant>({
    ...
    endpoint: `/admin/tenants/${editTenant?.id}`,
    enabled: !!editTenant,
  });
  ...
  const { data: membersData, isLoading: membersLoading } = useApiListQuery<TenantMember>({
    ...
    endpoint: `/admin/tenants/${membersTenant?.id}/members`,
  ...
  const updateMutation = useApiMutation<Tenant, Record<string, unknown>>({
    method: 'put',
    endpoint: (vars) => `/admin/tenants/${vars.id as string}`,
    invalidateKeys: [['admin', 'tenants']],
    onSuccess: () => {
  ...
  const enableMutation = useApiMutation<Tenant, { id: string }>({
    method: 'post',
    endpoint: (vars) => `/admin/tenants/${vars.id}/enable`,
    invalidateKeys: [['admin', 'tenants']],
    onSuccess: () => {
  ...
  const disableMutation = useApiMutation<Tenant, { id: string }>({
    method: 'post',
    endpoint: (vars) => `/admin/tenants/${vars.id}/disable`,
    invalidateKeys: [['admin', 'tenants']],
    onSuccess: () => {
  ...
  const inviteMemberMutation = useApiMutation<TenantMember, Record<string, unknown>>({
    method: 'post',
    endpoint: (vars) => `/admin/tenants/${vars.tenantId as string}/members`,
  ...
  const removeMemberMutation = useApiMutation<void, { tenantId: string; userId: string }>({
    method: 'delete',
    endpoint: (vars) => `/admin/tenants/${vars.tenantId}/members/${vars.userId}`,
    invalidateKeys: [['admin', 'tenants', membersTenant?.id, 'members']],
    onSuccess: () => {
  ...
  >({
    method: 'put',
    endpoint: (vars) => `/admin/tenants/${vars.tenantId}/members/${vars.userId}/role`,
    invalidateKeys: [['admin', 'tenants', membersTenant?.id, 'members']],
    onSuccess: () => {
          ...
          <>
            ...
            <div style={{ marginTop: 20 }}>
              <Button
                ...
                onClick={() => {
                  setViewTenant(null);
                  window.location.href = `/admin/tenants/${viewTenant.id}/usage`;
                }}
              >
                查看用量统计
              </Button>
